...
folder are mapped to the application's root namespace, 'CordovaPluginTest'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "CordovaPluginTest"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'CordovaPluginTest' in the same way
as `"CordovaPluginTest/sass/src"`.
